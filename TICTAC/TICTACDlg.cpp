...
CTICTACDlg::CTICTACDlg(CWnd* pParent /*=NULL*/)
...
void CTICTACDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	if (lock)
	{
		gr.playerA(point, this);
		lock = false;
	}
...
void CTICTACDlg::OnRButtonDown(UINT nFlags, CPoint point)
{
	if (!lock)
	{
		gr.playerB(point, this);
		lock = true;
	}
